...
possible_ip = list(map(lambda s: s[5:], subprocess.check_output("ip addr | grep -o -E 'inet 192[^/]*|inet 127[^/]*'", shell=True).decode("latin-1").split("\n")[:-1]))
...
def start_proxy(listen_port):
    if strace:
        proxy = pexpect.spawn("bash -c 'strace -f -tt ./proxy %d 2>>proxy-strace.log'" % (listen_port), timeout=DEFAULT_TIMEOUT, logfile=open("proxy.log", "a"), encoding='latin-1')
    else:
        proxy = pexpect.spawn("bash -c './proxy %d 2>&1'" % (listen_port), timeout=DEFAULT_TIMEOUT, logfile=open("proxy.log", "a"), encoding='latin-1')
...
def helper_send(proc, s):
    ...
    proc.sendline(binascii.hexlify(s.encode('latin-1')).decode('ascii'))
...
def helper_readline(proc, timeout=-1):
    ...
    if resp == "error\r\n":
        ...
        try:
            return binascii.unhexlify(resp.strip()).decode('latin-1')
        except:
            print (repr(resp))
            raise
...
def random_status():
    return random.choice(list(status_desc.keys()))
...
def do_part1_5(server, server_port, client, proxy):
    ...
    req = HttpRequest("POST", server_ip, server_port, path, body)
    # Hack the send_req
    lag_point = [random.randint(1, len(req.raw) - 1) for i in range(5)]
    lag_point.sort()
    helper_send(client, req.raw[:lag_point[0]])
    time.sleep(1)
    for i in range(len(lag_point) - 1):
    ...
    resp = HttpResponse(random_status(), random_bytes(random.randint(300, 500)))
    lag_point = [random.randint(1, len(resp.raw) - 1) for i in range(5)]
    lag_point.sort()
...
def do_part2_1(server, server_port, client, proxy):
    ...
    for i in range(PART2_1_ROUND):
        ...
        cut_point = [random.randint(bigresp.header_end_end + 1, len(bigresp.raw) - 1) for i in range(PART2_1_CUT * 2 - 1)]
        ...
        for j in range(len(cut_point) // 2):
            if j == 0:
...
def do_part2_2(server, server_port, client, proxy):
    ...
    client_server_mapping = [i for i in range(CONCURRENCY)]
