...
parse_t parse_instr(char **ptr, instr_t **inst)
{
    ...
    instr_t *new_instr = find_instr(*ptr);
    if (new_instr == NULL)
    {
        return PARSE_ERR;
    }
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
    const reg_t *new_reg = find_register(*ptr);
    if (new_reg == NULL)
    {
        return PARSE_ERR;
    }
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    char *new_symbol = *ptr;
    int symbol_lenth = 0;
    while (IS_LETTER(new_symbol) || IS_DIGIT(new_symbol))
    {
        symbol_lenth++;
        new_symbol++;
    }
    *name = malloc(symbol_lenth + 1);
    strncpy(*name, *ptr, symbol_lenth);
    *(*name + symbol_lenth) = '\0';
    /* set 'ptr' and 'name' */
    *ptr = new_symbol;
    return PARSE_SYMBOL;
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    char *new_lable = *ptr;
    int lable_lenth = 0;
    while (!IS_BLANK(new_lable) && !IS_END(new_lable))
    {
        if (*new_lable == ':')
        {
            *name = malloc(lable_lenth + 1);
            strncpy(*name, *ptr, lable_lenth);
            *(*name + lable_lenth) = '\0';
            *ptr = new_lable + 1;
            return PARSE_LABEL;
        }
...
int relocate(void)
{
    ...
    {
        /* find symbol */
        symbol_t *sym = find_symbol(reltab->name);
        if (sym == NULL)
        {
            err_print("Unknown symbol:'%s'", reltab->name);
            return -1;
        }
        /* relocate y64bin according itype */
        switch (reltab->y64bin->bytes)
        {
        case 1:
        case 2:
        case 4:
        case 8:
            for (int i = 0; i < reltab->y64bin->bytes; i++)
            {
                reltab->y64bin->codes[i] = (sym->addr >> (i * 8)) & 0xFF;
            }
            break;
        case 9:
        case 10:
            for (int i = 0; i < 8; i++)
            {
                reltab->y64bin->codes[reltab->y64bin->bytes - 8 + i] = (sym->addr >> (i * 8)) & 0xFF;
            }
            break;
        default:
            return -1;
...
int binfile(FILE *out)
{
    ...
    while (ltmp)
    {
        if (ltmp->type == TYPE_INS)
        {
            ...
            fwrite(ltmp->y64bin.codes, ltmp->y64bin.bytes, 1, out);
        }
        ltmp = ltmp->next;
    }
    return 0;
