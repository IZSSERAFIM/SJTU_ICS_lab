...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    bool_t ovf = FALSE;
    // long_t signA = (argA >> 63) & 0x1;
    // long_t signB = (argB >> 63) & 0x1;
    // long_t signV = (val >> 63) & 0x1;
    
    switch (op)
    {
    case A_ADD:
        if ((argA > 0 && argB > 0 && (int)val < 0)||(argA < 0 && argB < 0 && (int)val > 0))
        // if (val - argA != argB)
        // if ((signA == signB) && (signA != signV))
        // if (!(signA ^ signB) && (signB ^ signV))
            ovf = TRUE;
        break;
    case A_SUB:
        if ((argA < 0 && argB > 0 && (int)val < 0)||(argA > 0 && argB < 0 && (int)val > 0))
    ...
    // ovf = ((op == A_ADD && !(argA^argB) && (argB^val)) || (op == A_SUB && (argA^argB) && (argB^val)));
...
stat_t nexti(y64sim_t *sim)
{
    ...
    {
        if (!get_byte_val(sim->m, next_pc, &regids))
        {
            err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regA = GET_REGA(regids);
        regB = GET_REGB(regids);
        next_pc++;
    }
